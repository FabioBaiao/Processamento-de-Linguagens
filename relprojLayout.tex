...
Para esta \textit{query} adicionou-se uma funcionalidade que permite saber, a cada local, o número de vezes que este foi utilizado como local de saída, bem como o tipo de serviço: \textbf{saídas[tipo][saída]} representa o número de vezes que a saída \textit{saída} foi usada para o serviço \textit{tipo} (Portagem ou Parque de Estacionamento).
...
Nesta \textit{query} decidiu-se generalizar, apresentando os gastos mensais para todos os tipos existentes no extrato. 
Tal como no cálculo do total gasto no mês, também aqui os "totais" são calculados somando as importâncias e subtraindo os descontos.
...
Mais uma vez, inverte-se a data para permitir, posteriormente, a ordenação. Em cada transação calcula-se o valor gasto, somando-o ao gasto, para a data e tipo dessa transação, já calculado. 
...
Tal como nas outras queries, para apresentar os resultados ordenados pela data é necessário invertê-la. Neste caso, as datas utilizadas são as datas referentes à tag DATA_DEBITO.
